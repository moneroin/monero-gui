; Monero Infinity Beryllium Bullet GUI Wallet Installer for Windows
; Copyright (c) 2014-2018, The Monero Infinity Project
; See LICENSE

[Setup]
AppName=Monero Infinity GUI Wallet
; For InnoSetup this is the property that uniquely identifies the application as such
; Thus it's important to keep this stable over releases
; With a different "AppName" InnoSetup would treat a mere update as a completely new application and thus mess up

AppVersion=1.0.0.0
DefaultDirName={pf}\Monero Infinity GUI Wallet
DefaultGroupName=Monero Infinity GUI Wallet
...
LicenseFile=LICENSE
AppPublisher=The Monero Infinity Developer Community
...
; Background info:
; This installer indeed asks for admin rights so the Monero Infinity files can be copied to a place where they have at least
...
; As of version 0.13.0.4, the Monero Infinity .exe files do not carry version info anyway in their .exe headers.
...
; applications somehow. Luckily this is no issue here because ALL files are "private" to Monero Infinity.

Source: "ReadMe.htm"; DestDir: "{app}"; Flags: ignoreversion
Source: "FinishImage.bmp"; Flags: dontcopy

; Monero Infinity GUI wallet exe and guide
...
; Monero Infinity GUI wallet log file
...
; Monero Infinity wallet RPC interface implementation
Source: "bin\monero-wallet-rpc.exe"; DestDir: "{app}"; Flags: ignoreversion

; Monero Infinity daemon
Source: "bin\moneroinfinityd.exe"; DestDir: "{app}"; Flags: ignoreversion

; Monero Infinity daemon wrapped in a batch file that stops before the text window closes, to see any error messages
Source: "monero-daemon.bat"; DestDir: "{app}"; Flags: ignoreversion;

; Monero Infinity blockchain utilities
...
begin
  ...
  s := 'The default folder to store the Monero Infinity blockchain is ' + blockChainDefaultDir;
  ...
  if blockChainDir = '' then begin
    // Unfortunately 'TInputDirWizardDirPage' does not allow empty field, so "propose" Monero Infinity default location
...
begin
  if CurStep = ssPostInstall then begin
    ...
    SetArrayLength(s, 3);
    s[0] := 'REM Execute the Monero Infinity daemon and then stay with window open after it exits';
    s[1] := '"' + ExpandConstant('{app}\moneroinfinityd.exe') + '" ' + DaemonFlags('');
    s[2] := 'PAUSE';
...
[Icons]
; Icons in the "Monero Infinity GUI Wallet" program group
...
Name: "{group}\Utilities\Monero Infinity Daemon"; Filename: "{app}\moneroinfinityd.exe"; Parameters: {code:DaemonFlags}
...
Name: "{group}\Utilities\Textual (CLI) Wallet"; Filename: "{app}\monero-wallet-cli.exe"; WorkingDir: "{userdocs}\Monero Infinity\wallets"
...
Name: "{group}\Utilities\x (Check Default Wallet Folder)"; Filename: "{win}\Explorer.exe"; Parameters: "{userdocs}\Monero Infinity\wallets"
...
Name: "{group}\Utilities\x (Try Kill Daemon)"; Filename: "Taskkill.exe"; Parameters: "/IM moneroinfinityd.exe /T /F"
