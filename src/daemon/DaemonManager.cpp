...
bool DaemonManager::start(const QString &flags, NetworkType::Type nettype, const QString &dataDir, const QString &bootstrapNodeAddress)
{
    ...
    qDebug() << "starting moneroinfinityd " + m_monerod;
    ...
    while(true && !m_app_exit) {
        ...
        if(running(nettype)) {
            ...
            if(counter >= 5) {
                qDebug() << "Killing it! ";
#ifdef Q_OS_WIN
                QProcess::execute("taskkill /F /IM moneroinfinityd.exe");
#else
                QProcess::execute("pkill moneroinfinityd");
#endif
            }
...
#ifdef Q_OS_WIN
    m_monerod = QApplication::applicationDirPath() + "/moneroinfinityd.exe";
#elif defined(Q_OS_UNIX)
    m_monerod = QApplication::applicationDirPath() + "/moneroinfinityd";
#endif
